...
var reShortToken = regexp.MustCompile(`^[a-zA-Z0-9]{10}$`)

// validShortToken is a middleware ensuring token is exactly 10 alphanumeric characters.
func (a *App) validShortToken(next echo.HandlerFunc) echo.HandlerFunc {
	return func(c echo.Context) error {
		token := c.Param("token")
		if !reShortToken.MatchString(token) {
...
// ShortLinkRedirect handles compact 10-character short tracking URLs (https://host/link/{token}).
func (a *App) ShortLinkRedirect(c echo.Context) error {
	token := c.Param("token")
	if len(token) < 10 {
		return echo.ErrNotFound
	}

	payload, err := a.core.DecodeShortLink(token)
	if err != nil || payload.LinkID <= 0 {
		return c.Render(http.StatusNotFound, tplMessage, makeMsgTpl(a.i18n.T("public.errorTitle"), "", a.i18n.Ts("public.invalidLink")))
	}

	url, err := a.core.ResolveLinkURL(payload.LinkID)
	if err != nil || url == "" {
		return c.Render(http.StatusNotFound, tplMessage, makeMsgTpl(a.i18n.T("public.errorTitle"), "", a.i18n.Ts("public.invalidLink")))
	}

	// If tracking is globally disabled, redirect directly without logging click
	if a.cfg.Privacy.DisableTracking {
		return c.Redirect(http.StatusTemporaryRedirect, url)
	}
	...
	var stepIDPtr *int
	if payload.StepID > 0 {
		stepIDPtr = &payload.StepID
	} else if stepParam := c.QueryParam("step_id"); stepParam != "" {
		if sID, err := strconv.Atoi(stepParam); err == nil && sID > 0 {
			stepIDPtr = &sID
		}
	}
	...
	// Anonymize subscriber_id if IndividualTracking is disabled
	subID := payload.SubscriberID
	if !a.cfg.Privacy.IndividualTracking {
		subID = 0
	}

	_ = a.core.RegisterShortLinkClick(payload, clientMeta, subID)

	// Record sequence click strictly for genuine human activity
	if payload.IsSequence && payload.EntityID > 0 && payload.SubscriberID > 0 {
		if clientMeta.BotType != core.BotTypeHoneypot && clientMeta.BotType != core.BotTypeSecurityScanner {
			_ = a.core.RecordSequenceClick(payload.EntityID, payload.SubscriberID)
		}
	}
...
func (a *App) RegisterCampaignView(c echo.Context) error {
	...
	variantID := c.QueryParam("v")
	var stepIDPtr *int
	if stepParam := c.QueryParam("step_id"); stepParam != "" {
		if sID, err := strconv.Atoi(stepParam); err == nil && sID > 0 {
			stepIDPtr = &sID
		}
	}
	...
	if campUUID != dummyUUID && reqSubUUID != dummyUUID {
		...
		// Record sequence read for human activity and genuine mail proxies (excluding security scanners, crawlers, and honeypots)
		if !clientMeta.IsBot || clientMeta.IsProxy || clientMeta.BotType == core.BotTypeGoogleProxy || clientMeta.BotType == core.BotTypeProxyMPP {
			if sub, err := a.core.GetSubscriber(0, reqSubUUID, ""); err == nil {
				if seq, err := a.core.GetSequence(0, campUUID); err == nil {
