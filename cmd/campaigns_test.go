...
func TestE2E_Campaign_MultiStep_MultiStep_Execution(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_ConditionalRouting_IfRead(t *testing.T) {
	now := time.Now()
...
func TestE2E_Campaign_MultiStep_LinearProgression_And_LinkRedirection(t *testing.T) {
	now := time.Now()
...
func TestE2E_Campaign_MultiStep_Sender_Reassignment_And_Limits(t *testing.T) {
	contact := models.CampaignSubscriber{
		CampaignID:   400,
		SubscriberID: 401,
...
func TestE2E_Campaign_MultiStep_Schedule_Timezone_Pacing(t *testing.T) {
	...
	loc1 := contact1.ResolveTimezone()
	if loc1.String() != "Asia/Kolkata" {
		t.Errorf("expected Asia/Kolkata for Alice, got %s", loc1.String())
	}

	loc2 := contact2.ResolveTimezone()
	if loc2.String() != time.Local.String() {
		t.Errorf("expected %s for Bob, got %s", time.Local.String(), loc2.String())
	}
...
func TestE2E_Campaign_MultiStep_MultiStep_LLM_Lifecycle(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_REST_API_Pipeline(t *testing.T) {
	seq := models.Campaign{
		Base: models.Base{
			ID: 101,
		},
...
func TestE2E_Campaign_MultiStep_Outside_Window_Schedule_Deferral(t *testing.T) {
	...
	steps := []models.CampaignStep{
		{
			...
			Body:       "ðŸ›¸ *Incoming Transmission from HQ...*\n\nHey {{ .Subscriber.FirstName }}! We have a top-secret mission prepared for {{ .Subscriber.Email }}.\n\nðŸ‘ï¸ Leave this chat unread and nothing happens... Open it to give us the Blue Ticks, and weâ€™ll immediately beam the payload to your inbox!",
		},
		{
			StepNumber: 2,
			Delay:      "0s",
			...
			Body:       "We just beamed an urgent mission email to {{ .Subscriber.Email }}! ðŸ›¸\n\nðŸƒâ€â™‚ï¸ Sprint over to your inbox and click the button before carrier pigeons eat the bandwidth!",
		},
		{
			StepNumber: 3,
			Delay:      "10s",
			Messenger:  "email",
			Condition:  models.CampaignConditionAlways,
			Subject:    "ðŸ§ª [Team Demo] Click this link to trigger Step 4 on WhatsApp!",
			Body:       "<p>Hi {{ .Subscriber.FirstName }}!</p><p>You triggered the <code>if_read</code> Blue Tick response!</p><p><a href=\"https://example.com/demo@TrackLink\">ðŸ‘‰ CLICK ME TO TRIGGER WHATSAPP STEP 4 ðŸ‘ˆ</a></p>",
		},
		{
			StepNumber: 4,
			Delay:      "0s",
			...
			Body:       "ðŸŽ¯ *CLICK EVENT REGISTERED IN REAL-TIME!*\n\n{{ .Subscriber.FirstName }}, you clicked the button like a 10x engineer! ðŸª Listmonk saw your click immediately.",
		},
		{
			StepNumber: 5,
			Delay:      "45s",
		...
		},
		{
			...
			Messenger:  "email",
			Condition:  models.CampaignConditionAlways,
			Subject:    "ðŸ† [Demo Complete] You conquered the 2-minute sequence!",
			Body:       "<h2>ðŸŽ‰ Demo Complete!</h2><p>You have tested WAHA Blue Tick reads, email handoffs, and link clicks in under 2 minutes.</p>",
		},
	}
...
func TestInstall_SeededResources_Structure(t *testing.T) {
	...
	steps := []models.CampaignStep{
		...
		},
		{
			...
			Body:       "<p>Hi {{ .Subscriber.FirstName }}!</p><p>Here is Step 2 with your personalized platform link:</p><p><a href=\"https://listmonk.app@TrackLink\">ðŸ‘‰ Click here to explore the platform ðŸ‘ˆ</a></p><p>We will check in with you shortly on WhatsApp!</p>",
			EmailType:  models.EmailTypeNewThread,
...
func TestE2E_Campaign_MultiStep_ListBasedTrigger_Enrollment(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_ListEnrollment_MultiList_Overlap(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_Activity_Heading_Format(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_Email_MailHog_Lifecycle(t *testing.T) {
	mailhogHTTP := getEnv("MAILHOG_HTTP_URL", "http://localhost:8025")
	mailhogSMTPHost := getEnv("MAILHOG_SMTP_HOST", "localhost")
	mailhogSMTPPort := 1025
...
func TestE2E_Campaign_MultiStep_WAHA_WhatsApp_Lifecycle(t *testing.T) {
	testutil.LoadDotEnv()
	wahaURL := getEnv("WAHA_HOST", "http://localhost:3000")
	apiKey := getEnv("WAHA_API_KEY", "")
	...
	if isLive {
		...
		msg1 := models.Message{
			...
			MessengerSession: senderSess.Name,
			Subject:          step1.Subject,
			Body:             []byte(fmt.Sprintf("ðŸ§ª *TEST:* TestE2E_WhatsAppSequenceLifecycle\nðŸ“ *SUITE:* E2E/Sequences\nðŸŽ¯ *ACTION:* Sequence Step 1 Dispatch (Direct WhatsApp Text)\nðŸ‘¤ *RECIPIENT:* Contact {{ .Subscriber.Name }}\nâœ… *EXPECTED:* Message ACK receipt, last_read_at timestamp update\nðŸ”— *TRACKED LINK:* %s", "http://localhost:9000/r/waha-seq-deal")),
			Subscriber: models.Subscriber{
				Base:  models.Base{ID: subID},
				Phone: null.StringFrom(receiverSess.Phone),
				Name:  "Test Contact",
...
func TestE2E_Campaign_MultiStep_Mixed_Messenger_Lifecycle(t *testing.T) {
	testutil.LoadDotEnv()
	mailhogHTTP := getEnv("MAILHOG_HTTP_URL", "http://localhost:8025")
	mailhogSMTPHost := getEnv("MAILHOG_SMTP_HOST", "localhost")
	mailhogSMTPPort := 1025
	...
	if err == nil && isWAHALive {
		msg2 := models.Message{
			...
			MessengerSession: senderSess.Name,
			Subject:          step2.Subject,
			Body:             []byte(fmt.Sprintf("ðŸ§ª *TEST:* TestE2E_MixedSequenceLifecycle\nðŸ“ *SUITE:* E2E/MixedSequences\nðŸŽ¯ *ACTION:* Sequence Step 2 Dispatch (WhatsApp Follow-Up)\nðŸ‘¤ *RECIPIENT:* Contact {{ .Subscriber.Name }}\nâœ… *EXPECTED:* Cross-channel sender stickiness (User Alice session waha-alice-1)\nðŸ”— *TRACKED LINK:* %s", "http://localhost:9000/r/mixed-exclusive")),
			Subscriber: models.Subscriber{
				Base:  models.Base{ID: subID},
				Phone: null.StringFrom(receiverSess.Phone),
			},
...
func TestE2E_Campaign_MultiStep_TestMessage_MailHog_And_WAHA_Routing(t *testing.T) {
	testutil.LoadDotEnv()
	mailhogHTTP := getEnv("MAILHOG_HTTP_URL", "http://localhost:8025")
	wahaURL := getEnv("WAHA_HOST", "http://localhost:3000")
	apiKey := getEnv("WAHA_API_KEY", "")
	...
	testWhatsAppReq := sequenceTestReq{
		...
		Body:             "ðŸ§ª *TEST:* TestE2E_Campaign_MultiStep_TestMessage_MailHog_And_WAHA_Routing\nðŸ“ *SUITE:* E2E/Sequences\nðŸŽ¯ *ACTION:* Test Message Delivery Verification\nðŸ‘¤ *RECIPIENT:* WhatsApp Target\nâœ… *EXPECTED:* Successful routing to WAHA messenger",
...
func TestE2E_Campaign_MultiStep_EnrollSubscribersByList_SQLTypeSafety(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_StepCompilation_And_FromEmail_MailHog(t *testing.T) {
	mailhogHTTP := getEnv("MAILHOG_HTTP_URL", "http://localhost:8025")
	mailhogSMTPHost := getEnv("MAILHOG_SMTP_HOST", "localhost")
	mailhogSMTPPort := 1025
...
func TestE2E_Campaign_MultiStep_TestMessage_ActiveUserRouting_And_ShorthandTemplateRendering(t *testing.T) {
	// Setup production contact
	contactSub := models.Subscriber{
	...
	step := models.CampaignStep{
		...
		Body:       "<p>Hi {{ .Subscriber.FirstName }}!</p><p>You triggered step for {{ .Subscriber.Attribs.company }}.</p><p><a href=\"https://example.com/demo@TrackLink\">ðŸ‘‰ CLICK LINK ðŸ‘ˆ</a></p>",
	}
...
func TestE2E_Campaign_MultiStep_ProductionMessage_Routing_And_ContactRendering(t *testing.T) {
	// Setup production contact
	contactSub := models.Subscriber{
		Base:  models.Base{ID: 202},
		Name:  "Jane Doe",
...
func TestE2E_Campaign_MultiStep_RealWorld_Pooled_LoadBalancing(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_BaseTemplate_L_Function_Integration(t *testing.T) {
...
func TestE2E_Campaign_MultiStep_WhatsApp_HTMLTemplate_BypassAndSanitization(t *testing.T) {
	...
	step := models.CampaignStep{
		...
		Messenger:  "whatsapp",
		Subject:    "Step 1",
		Body:       "ðŸ›¸ *Incoming Transmission from HQ...* Hey {{ .Subscriber.Name }}! We have a top-secret mission prepared for {{ .Subscriber.Email }}. &nbsp;&nbsp; View in browser",
	}
...
func TestE2E_Campaign_MultiStep_TeamDemo_RealTimeClickTrigger(t *testing.T) {
