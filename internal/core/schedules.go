...
func (c *Core) GetSchedules() ([]models.Schedule, error) {
	...
	err := c.db.Select(&out, "SELECT id, uuid, name, timezone, use_subscriber_timezone, skip_holidays, sending_windows, is_default, created_at, updated_at FROM schedules ORDER BY id DESC")
	if err != nil {
...
func (c *Core) GetSchedule(id int, uid string) (*models.Schedule, error) {
	...
	err := c.db.Get(&s, "SELECT id, uuid, name, timezone, use_subscriber_timezone, skip_holidays, sending_windows, is_default, created_at, updated_at FROM schedules WHERE id = $1 OR uuid::text = $2", id, uid)
	if err != nil {
...
func (c *Core) CreateSchedule(s models.Schedule) (*models.Schedule, error) {
	...
	var out models.Schedule
	err := c.db.Get(&out, `INSERT INTO schedules (uuid, name, timezone, use_subscriber_timezone, skip_holidays, sending_windows)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id, uuid, name, timezone, use_subscriber_timezone, skip_holidays, sending_windows, created_at, updated_at`,
		s.UUID, s.Name, s.Timezone, s.UseSubscriberTimezone, s.SkipHolidays, s.SendingWindows)
	if err != nil {
...
func (c *Core) UpdateSchedule(s models.Schedule) (*models.Schedule, error) {
	...
	_, err := c.db.Exec(`UPDATE schedules
		...
		WHERE id = $1`,
		s.ID, s.Name, s.Timezone, s.UseSubscriberTimezone, s.SkipHolidays, s.SendingWindows)
	if err != nil {
