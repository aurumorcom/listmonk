...
func (c *Core) GetSettings() (models.Settings, error) {
	...
	if err == nil && len(emailAccounts) > 0 {
		...
		for _, em := range emailAccounts {
			...
			imapMap, _ := smtpMap["imap"].(map[string]any)
			...
			entry := models.SMTPSettings{
				...
				IMAPEnabled:       getBool(imapMap, "enabled", getBool(smtpMap, "imap_enabled", false)),
				IMAPHost:          getString(imapMap, "host", getString(smtpMap, "imap_host", "")),
				IMAPPort:          getInt(imapMap, "port", getInt(smtpMap, "imap_port", 993)),
				IMAPAuthProtocol:  getString(imapMap, "auth_protocol", getString(smtpMap, "imap_auth_protocol", "login")),
				IMAPUsername:      getString(imapMap, "username", getString(smtpMap, "imap_username", "")),
				IMAPPassword:      getString(imapMap, "password", getString(smtpMap, "imap_password", "")),
				IMAPFolder:        getString(imapMap, "folder", getString(smtpMap, "imap_folder", "INBOX")),
				IMAPTLSType:       getString(imapMap, "tls_type", getString(smtpMap, "imap_tls_type", "TLS")),
				IMAPTLSSkipVerify: getBool(imapMap, "tls_skip_verify", getBool(smtpMap, "imap_tls_skip_verify", false)),
				IMAPMaxConns:      getInt(imapMap, "max_conns", getInt(smtpMap, "imap_max_conns", 5)),
				IMAPIdleTimeout:   getString(imapMap, "idle_timeout", getString(smtpMap, "imap_idle_timeout", "15s")),
				IMAPWaitTimeout:   getString(imapMap, "wait_timeout", getString(smtpMap, "imap_wait_timeout", "5s")),
				IMAPMaxRetries:    getInt(imapMap, "max_retries", getInt(smtpMap, "imap_max_retries", 3)),
				IMAPRetryDelay:    getString(imapMap, "retry_delay", getString(smtpMap, "imap_retry_delay", "30s")),
				IMAPInterval:      getString(imapMap, "interval", getString(smtpMap, "imap_interval", "30s")),
			}
		...
		for _, item := range s.SMTP {
			...
			smtpCfg := models.JSON{
				...
				"imap": map[string]any{
					...
					"host":            item.IMAPHost,
					"port":            item.IMAPPort,
					"auth_protocol":   item.IMAPAuthProtocol,
					"username":        item.IMAPUsername,
					"password":        item.IMAPPassword,
					"folder":          item.IMAPFolder,
					"tls_type":        item.IMAPTLSType,
					"tls_skip_verify": item.IMAPTLSSkipVerify,
					"max_conns":       item.IMAPMaxConns,
					"idle_timeout":    item.IMAPIdleTimeout,
					"wait_timeout":    item.IMAPWaitTimeout,
					"max_retries":     item.IMAPMaxRetries,
					"retry_delay":     item.IMAPRetryDelay,
					"interval":        item.IMAPInterval,
				},
				"imap_enabled":         item.IMAPEnabled,
				"imap_host":            item.IMAPHost,
				"imap_port":            item.IMAPPort,
				"imap_auth_protocol":   item.IMAPAuthProtocol,
				"imap_username":        item.IMAPUsername,
				"imap_password":        item.IMAPPassword,
				"imap_folder":          item.IMAPFolder,
				"imap_tls_type":        item.IMAPTLSType,
				"imap_tls_skip_verify": item.IMAPTLSSkipVerify,
				"imap_max_conns":       item.IMAPMaxConns,
				"imap_idle_timeout":    item.IMAPIdleTimeout,
				"imap_wait_timeout":    item.IMAPWaitTimeout,
				"imap_max_retries":     item.IMAPMaxRetries,
				"imap_retry_delay":     item.IMAPRetryDelay,
				"imap_interval":        item.IMAPInterval,
			}
